...
- **Persistance de Sélection et Actions Batch (v0.16.7)** :
  - **Identifiant unique par UUID natif (aria-labelledby / jslog)** : Pour contrer les reconstructions de cartes DOM et gérer les doublons de titres de manière infaillible, l'extension extrait l'UUID unique natif Google de chaque carte (attribut `aria-labelledby` pour les notes, `jslog` pour les artéfacts) et l'utilise comme unique clé de sélection et de suppression.
  - **Bouton Tout Désélectionner (×)** : Introduction d'un bouton de réinitialisation rapide à côté du bouton de suppression par lot du Studio. Son clic vide instantanément l'état interne de sélection (`selectedUuids.clear()`) et décoche toutes les checkboxes actuellement visibles dans le DOM du Studio.
  - **Mapping RPC direct** : Lors du clic sur "Supprimer la sélection", les requêtes de suppression RPC (`AH0mwd` et `V5N4be`) sont générées directement à partir des UUIDs réels stockés dans le `Set`, éliminant toute comparaison approximative par titre.
  - **Parade anti-recyclage Angular** : L'extension applique un effet de fondu doux opacité 0 + visibilité cachée sur la carte supprimée. Au moment de l'injection ou de la ré-hydratation par l'observateur, si Angular a réutilisé un composant DOM pour afficher une note restante, les styles d'opacité et de visibilité de l'hôte sont réinitialisés, assurant l'auto-guérison instantanée du DOM.
